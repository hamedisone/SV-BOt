...
if matches[1] == "بای" then
     ...
     if msg.to.type == 'channel' or 'chat' then
           local answers = {'بی ادب','تیرشو یادت رفت','حضوری بیا بگامت بچه پرو','اگه نرم میخوای چه غلطی بکنی مثلا','چته الان دقیقا','خخخخخ بسیک باو'}
            return answers[math.random(#answers)]
      end
	   end
      ...
      if matches[1] == "اسپم" then
      if msg.to.type == 'channel' or 'chat' then
            local answers = {'@DelNeveshtBoD اسپم'}
            return answers[math.random(#answers)]
      end
	  end
	  if matches[1] == "bk" then
     if msg.to.type == 'channel' or 'chat' then
           local answers = {'bk k bk','mage dari?','به نخودت نناز خخخخ','بی شرف نفرست','نفرست عوضی','نزن کله خمیری','بیا خونمون','ارسال نکن','بسسسسسسسسسسسهههههههههههههسههههههههه'}
            return answers[math.random(#answers)]
      end
    end
end
