...
function run(msg, matches)
	...
	if matches[1]:lower() == 'expire' then
		...
		if not expiretime then return 'تاریخ ست نشده است' else
			local now = tonumber(os.time())
			return (math.floor((tonumber(expiretime) - tonumber(now)) / 86400) + 1) .. " روز دیگر تا پایان اعتبار شما باقی مانده است جهت تمدید پیام دهید @Hamed_Server"
		end
	end
